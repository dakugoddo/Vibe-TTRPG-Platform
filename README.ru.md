...
Бета-версия пригодна для локального тестирования и ранней подготовки кампаний, но ещё не является полированным публичным релизом. Основная архитектура уже есть; UI, права доступа, упаковка и multiplayer QA всё ещё дорабатываются.
...
- Obsidian-подобная рабочая область Notes: вкладки-редакторы, разделение панелей, перетаскиваемые shell-модули, сохранение раскладки и сброс.
- Вложенные заметки как встроенные блоки; Markdown-вставки (`![[...]]`) и умное автодополнение wiki-ссылок.
- Настраиваемая раскладка листа заметки (публикация, применение, откат, сброс) с безопасной доставкой игрокам.
- Полная русско-английская локализация интерфейса и редактор мировых переводов, чтобы ГМ мог переопределять строки внутри мира.
- Вставка из буфера обмена на канвасе (текст → прямоугольник, изображения → ассеты); более плавные кривые и взаимодействие в стиле Excalidraw.
- Возобновляемая по частям загрузка файлов более 20 МБ и нативный модуль предпросмотра PDF.
- Единая система тем (семантические токены) для оболочки, окон сущностей и карточек.
- Компактная карточка персонажа на канвасе с вкладками и быстрыми бросками; лист способностей.
- Профили ролей игроков и более точное разделение прав ГМ/игрок для действий с сущностями.
- Desktop: собственная иконка приложения, ленивая загрузка экранов, стабильный production build.
