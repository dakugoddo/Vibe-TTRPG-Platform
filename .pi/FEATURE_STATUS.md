...
- В том же меню `EntityWindow` доступны quick-create действия для текущих flexible blocks: у персонажа можно создать дочерний предмет или компетенцию, у предмета — дочернюю атаку; созданная сущность сразу открывается в окне.
