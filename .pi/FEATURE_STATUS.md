...
- Select-drag draw-elements оптимизирован: live-перемещение работает как локальный preview, persistent Yjs получает только финальную позицию на drag end.
...
- Point edit, resize, rotate и fog brush пока требуют отдельного performance-прохода, потому что всё ещё могут писать live-изменения через Yjs.
