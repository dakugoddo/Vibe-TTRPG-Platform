...
| `FEAT-WORKSPACE-004` | `F1` | Workspace / Notes Panes | First leaf-open slice implemented, owner QA pending | Полноценная Obsidian-like leaf модель: каждая открытая сущность должна быть самостоятельным editor-window/leaf, drop в центр другого leaf объединяет сущности в одно окно с вкладками, drop на края создает row/column split, sidebars остаются отдельными dock-областями. Первый срез добавил `openNotesWorkspaceTabInNewLeaf()` / `openTabInNewLeaf`: клик по новой сущности открывает отдельный leaf рядом с активным, повторный клик фокусирует уже открытый leaf без дубля, center-drop остается merge в tab group, edge-drop остается split. | Ручная QA: открыть несколько сущностей из Vault/Search/ссылок, убедиться что они появляются отдельными editor leaves; перетащить leaf в центр другого для вкладок; перетащить на левый/правый/верхний/нижний край для split; затем polish визуального leaf-frame, если окно всё ещё ощущается как tab-strip. |
