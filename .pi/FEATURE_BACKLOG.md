...
| `FEAT-I18N-002` | `F1` | Localization / Modding | Foundation implemented, custom world layer planned | RU/EN switch, локальный preference, расширенный Settings/Notes UI перевод, desktop-кнопка открытия встроенной папки `locales`, App/Login shell, drawers/HUD, notifications, hotkeys, Asset Browser, image asset picker, Canvas Toolbar, entity link/drop popover, EntityDatabase context/group/search/bulk, Audio desk/dock, ChatPanel, EntityWindow shell, CharacterSheet tabs, SheetTabs, TagEditor/TagPicker, canvas token settings, InfiniteCanvas upload/delete/compact/portal shell, EntityImageBlock, SkillsBlock, CompetencyRollPopup, ResourcesBlock, PropertiesBlock, AbilitiesBlock, AbilitySheet, CompetenciesBlock, AttackSheet, InventoryBlock, ObjectSheet, AttributeBlock, MarkdownRenderer, StatTooltip и StyleDemo string pass есть. Оставшаяся кириллица в коде в основном относится к data/model compatibility, тестам и шаблонам новых сущностей, а не к стабильному UI chrome. Пользовательский редактор переводов нужен для моддеров. | Manual English smoke по основным экранам; design-doc для world locale format, endpoints, import/export и rollback. |
| `FEAT-MODULES-001` | `F0` | Modules / Modding | Design rule updated | Core vs optional module vs future mod/data-pack gate зафиксирован в `.pi/docs/module-architecture.md`. Core: мир, sync, permissions, notes/canvas/entity schema. Optional built-ins: audio, PDF, graph, combat tracker, automation, 3D. Future mods/data packs: systems, sheets, themes, translations, integrations. | Перед новой крупной функцией сначала классифицировать слой; не добавлять `AppModuleId` без реального mount point/service gate. |
...
| `FEAT-PDF-001` | `F2` | Asset Library / CanvasModule | First native asset preview slice implemented | PDF индексируется как asset type и открывается в Asset Browser через нативное превью без viewer-зависимости. Linked PDF card/page preview на canvas остаётся отдельным optional module slice. | Design-doc нужен перед zoom/search/page preview, canvas linked cards или тяжёлой viewer-зависимостью. |
