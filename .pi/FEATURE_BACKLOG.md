...
| `FEAT-WORKSPACE-004` | `F1` | Workspace / Notes Panes | Next architecture slice | Полноценная Obsidian-like leaf модель: каждая открытая сущность должна быть самостоятельным editor-window/leaf, drop в центр другого leaf объединяет сущности в одно окно с вкладками, drop на края создает row/column split, sidebars остаются отдельными dock-областями. Текущий `Editor`-модуль с внутренней tab-strip считается временным переходным слоем. | Спроектировать поверх `notesWorkspaceLayout.ts`: `WorkspaceNode` tree для editor leaves, entity tab groups как самостоятельные окна, center-drop merge, edge-drop split, resize handles между соседями, persisted ratios и focused QA по edge/center drop. |
