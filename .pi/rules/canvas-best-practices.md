...
### Line modes and binding

- `curved` не должен синтезировать midpoint для 2-точечной line/arrow. Две точки остаются прямым сегментом; плавное искривление появляется только после добавления опорной точки.
- Line/arrow binding хранится как `{ elementId, anchor, focus? }`. `focus` нужен для привязки к произвольной точке стороны объекта; старые binding-и без `focus` должны продолжать работать.
