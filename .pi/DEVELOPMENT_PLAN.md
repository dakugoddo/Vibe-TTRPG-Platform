...
7. Следующее обновление — архитектурный epic `Entity UI / Sheet Builder` (`FEAT-ENTITY-UI-BUILDER-001`):
   ...
   - следующий полезный срез — цельный GM workflow выбора/импорта note layout с validation preview и rollback; не добавлять отдельно storage/API/внутренние blocks, пока ими нельзя воспользоваться из production UI.
