...
- добавить быстрые шаблонные операции: дублирование сущности с дочерними элементами;
- добавить быстрые Knowledge Base операции: копирование `[[wiki-ссылки]]` из базы сущностей;
- добавить поиск по базе сущностей с сохранением дерева родителей/детей;
- добавить quick-create дочерних сущностей для текущих flexible blocks;
- сделать выдачу предметов и открытие связанных сущностей быстрее;
- добавить быстрые связи в `EntityWindow`: родитель, дочерние сущности, теги и обратные `[[wiki-ссылки]]`; ✅ Базово работает.
- расширить контекстное меню `EntityWindow` быстрыми действиями без возврата в базу: копировать `[[wiki-ссылку]]`, дублировать редактируемую сущность; ✅ Базово работает.
- добавить quick-create дочерних сущностей из `EntityDatabase` и `EntityWindow` для персонажа и предмета, включая flexible `ability`; ✅ Базово работает.
