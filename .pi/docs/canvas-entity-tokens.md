...
- Compact character card не хранит отдельный список выбранных полей: данные выводятся из `character` entity и дочерних сущностей, показываются на canvas card компактно со скроллом/вкладками и скрываются, если текущая роль не имеет права редактировать entity.
