...
Минимальный миграционный сценарий покрыт focused tests:

- client: `app/src/utils/entitySerializer.test.ts` проверяет чтение legacy `.md` без `schemaVersion`, нормализацию версии и запись `schemaVersion: 1`;
- server: `server/src/fileManager.test.ts` проверяет тот же сценарий для `general` и `user` баз, включая сохранение `uid` у user-сущностей.
