...
| Notes workspace mode | `app/src/utils/workspaceMode.ts`, `app/src/store/workspaceModeStore.ts`, `app/src/components/workspace/NotesWorkspace.tsx`, `app/src/App.tsx`, `app/src/components/ui/HudBar.tsx`, `.pi/docs/notes-workspace-mode-design.md` | local-only `Канвас/Заметки` mode, canvas render toggle, quick access to visible entities, unpinned screen-window workspace actions |
