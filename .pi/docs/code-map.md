...
| Localization runtime | `app/src/i18n.ts`, `app/src/utils/localization.ts`, `app/src/utils/worldLocaleRuntime.ts`, `app/src/hooks/useLocalePreference.ts`, `app/src/locales/ru.json`, `app/src/locales/en.json`, `app/src/services/fileApi.ts`, `app/src/components/ui/SettingsWindow.tsx`, `server/src/worldLocaleManager.ts`, `.pi/docs/i18n-custom-locales-foundation.md`, `.pi/docs/world-locale-editor-qa.md` | language preference, localStorage-backed locale choice, built-in ru/en bundles, flatten/unflatten/merge locale helpers, typed world locale list/read/write/rollback API, Settings discovery/preview/editor/import/export for world overrides, runtime application of supported `ru/en` world overrides |
