...
| Platform/native runtime | `.pi/docs/platform-runtime-decision.md`, `.pi/docs/tauri-native-migration-plan.md`, `app/package.json`, `server/package.json`, `server/src/index.ts` | Tauri/native gate, browser-first rollback, Vite build, Express/Yjs sidecar boundaries, future native windows and player delivery decisions |
...
| Compact character card on canvas | `app/src/utils/characterCardSummary.ts`, `app/src/utils/characterCompactCardDefaults.ts`, `app/src/utils/entityActionRollModel.ts`, `app/src/services/entityActionRoll.ts`, `app/src/components/canvas/InfiniteCanvas.tsx`, `app/src/components/windows/blocks/EntityCanvasTokenSettings.tsx`, `.pi/docs/compact-character-card-canvas.md`, `.pi/docs/compact-character-card-defaults-gate.md` | informational compact summary, вкладки `Статы/Действия/Ресурсы/Заметки`, entity-level visible field defaults in `properties.compactCard`; canvas overlay must not become an editor or roll surface |
...
| Notes workspace mode | `app/src/utils/workspaceMode.ts`, `app/src/utils/notesWorkspaceLayout.ts`, `app/src/utils/notesWorkspaceLinks.ts`, `app/src/store/workspaceModeStore.ts`, `app/src/store/notesWorkspaceStore.ts`, `app/src/components/workspace/NotesWorkspace.tsx`, `app/src/App.tsx`, `app/src/components/ui/HudBar.tsx`, `.pi/docs/notes-workspace-mode-design.md` | local-only `Канвас/Заметки` mode, canvas render toggle, quick access to visible entities, unpinned screen-window workspace actions, named local snapshots, persisted local tab/split board, drag/drop tabs, split pane resize ratio, read-only linked views |
