...
- Future editor leaf model: current `Editor` shell module with an internal tab board is transitional. `FEAT-WORKSPACE-004` requires entity editor leaves as independent windows that can merge into tab groups by center-drop and split by edge-drop; do not mark that feature done by only adding more buttons to the existing tab strip.
