...
| Localization runtime | `app/src/i18n.ts`, `app/src/utils/localization.ts`, `app/src/hooks/useLocalePreference.ts`, `app/src/locales/ru.json`, `app/src/locales/en.json`, `app/src/services/fileApi.ts`, `app/src/components/ui/SettingsWindow.tsx`, `server/src/worldLocaleManager.ts`, `.pi/docs/i18n-custom-locales-foundation.md` | language preference, localStorage-backed locale choice, built-in ru/en bundles, typed read-only world locale file discovery in Settings, future custom world locale editor plan |
