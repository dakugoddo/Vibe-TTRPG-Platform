...
| Settings shell | `app/src/components/ui/SettingsWindow.tsx`, `app/src/App.tsx`, `app/src/utils/permissions.ts`, `app/src/utils/devPerformanceOverlay.ts`, `.pi/docs/player-identity-roles.md` | окно настроек, player-safe tabs, GM-only world/roles tabs, role policy matrix, player profile role assignment, read-only world locale file preview, локальный reset center для theme/density, screen windows, notes layout, notes shell, audio defaults и dev-only performance overlay. Reset controls must never touch world `.md` data. |
...
| Localization runtime | `app/src/i18n.ts`, `app/src/utils/localization.ts`, `app/src/utils/worldLocaleRuntime.ts`, `app/src/hooks/useLocalePreference.ts`, `app/src/locales/ru.json`, `app/src/locales/en.json`, `app/src/services/fileApi.ts`, `app/src/store/yjsStore.ts`, `app/src/App.tsx`, `app/src/components/ui/SettingsWindow.tsx`, `server/src/worldLocaleManager.ts`, `.pi/docs/i18n-custom-locales-foundation.md`, `.pi/docs/world-locale-editor-qa.md` | language preference, localStorage-backed locale choice, built-in ru/en bundles, flatten/unflatten/merge locale helpers, typed world locale list/read/write/rollback API, Settings discovery/preview/key-table editor with filters/import/export for world overrides, host-only Yjs `worldLocales` snapshot publishing, player-side runtime application of supported `ru/en` world overrides without File API |
...
| Audio module shell | `app/src/components/ui/AudioControlDock.tsx`, `app/src/components/ui/AudioDesk.tsx`, `.pi/docs/gm-audio-desk.md` | отдельный нижний аудио-док, temporary mixer popup, local cue buttons, channel stop/all-stop, optional broadcast controls, bottom-dock empty/error status для музыки. `AudioControlDock` должен отправлять request-id в единственный `AudioDesk`, а не создавать второй playback owner, иначе Canvas/Notes switch может остановить звук. |
...
|-------|-----------------|
| `app/src/store/yjsStore.ts` | Yjs connection, roles, permissions gate for Entity mutations, chat messages, latest audio session command, host-published world locale snapshots |
...
| Canvas linked entity tokens/cards | `app/src/types/canvasTypes.ts`, `app/src/components/canvas/InfiniteCanvas.tsx`, `app/src/components/canvas/CanvasToolbar.tsx`, `app/src/utils/canvasEntityTokenFrame.ts`, `app/src/utils/entityCanvasDefaults.ts`, `app/src/components/windows/blocks/EntityCanvasTokenSettings.tsx`, `.pi/docs/canvas-entity-tokens.md` | Konva entity token/art rendering, token label bbox/ellipsis, card name/type strip, info affordance and context menu. Long names must be clipped inside the element bbox; do not let token labels render below the saved element height. |
| Compact character card on canvas | `app/src/utils/characterCardSummary.ts`, `app/src/utils/entityActionRollModel.ts`, `app/src/services/entityActionRoll.ts`, `app/src/components/canvas/InfiniteCanvas.tsx`, `app/src/components/windows/blocks/EntityCanvasTokenSettings.tsx`, `.pi/docs/compact-character-card-canvas.md` | informational compact summary on the canvas card itself: metrics, resources, actions, inventory and description are derived from entity/children and scroll inside the card; no `properties.compactCard` field selector/settings UI; canvas overlay must not become an editor or roll surface |
| Workspace/window ergonomics | `app/src/utils/windowLayout.ts`, `app/src/store/windowStore.ts`, `app/src/components/windows/WindowManager.tsx`, `app/src/components/windows/EntityWindow.tsx`, `.pi/docs/workspace-window-ergonomics-plan.md` | screen/canvas window layers, local layout actions, quick screen snapshot, current-room local reset for unpinned screen windows, future named/workspace snapshots and multi-monitor design gate |
| Notes workspace mode | `app/src/utils/workspaceMode.ts`, `app/src/utils/notesWorkspaceLayout.ts`, `app/src/utils/notesWorkspaceLinks.ts`, `app/src/utils/notesWorkspaceModules.ts`, `app/src/store/workspaceModeStore.ts`, `app/src/store/notesWorkspaceStore.ts`, `app/src/components/workspace/NotesWorkspace.tsx`, `app/src/App.tsx`, `app/src/components/ui/HudBar.tsx`, `app/src/components/ui/SettingsWindow.tsx`, `.pi/docs/notes-workspace-obsidian-redesign.md`, `.pi/docs/notes-workspace-rich-search-polish.md` | local-only `Канвас/Заметки` mode, canvas render toggle, shared theme background, grouped Obsidian-like vault tree, storage combobox, extended entity search via `entitySearch.ts`, persisted local tab/split editor board, drag/drop tabs with edge drop-zones, closeable panes, split pane resize ratio, `notesWorkspaceModules.ts` registry for implemented/planned shell modules including required non-toggleable `Editor`, persisted shell module visibility/area/order/area-layout/width/height for Editor/Vault/Context/Notifications/Search/Graph/Audio, SettingsWindow controls for toggleable shell module visibility plus `resetShell()` for shell-only defaults, Markdown-rich toolbar over Markdown source, source/preview/split/UI modes, entity data view, children/attached entities, outline/backlinks/graph context, active-entity quick actions for parent/copy wiki/copy id/pin to active canvas, active pane/tab affordances, stronger drop-zone overlay and empty pane state. `notesWorkspaceLayout.ts` owns both tab merge/split and `openNotesWorkspaceTabInNewLeaf()`; `notesWorkspaceStore.openTabInNewLeaf()` is the user-facing Notes entity-open action, while `openTab()` is only for explicit same-group tab behavior/tests. `notesWorkspaceStore.ts` owns shell storage versioning and must migrate unsafe legacy `moduleAreas/moduleOrder/moduleLayouts` before restoring layout. Do not reintroduce screen-window layout controls as the main notes workflow. Do not call `preventDefault()` on draggable pane/tab `mousedown`; row click must cover the whole entity row, chevron alone expands children. Notes shell visibility/size/area/order/layout belongs in `notesWorkspaceStore`, not component `useState`, and must not be reset with central `resetLayout()`. Audio in Notes mode uses the embedded host `NOTES_AUDIO_DOCK_HOST_ID`; `AudioControlDock` must remain the single mounted owner of `AudioDesk` to avoid duplicate playback lifecycles and audio stops on Canvas/Notes switches, while embedded mode must use compact chrome so only the shell module header is visible. |
...
- Notes working tab model: in Notes mode, entity leaves, shell modules and user-facing "windows" are the same UX concept: a movable working tab. They can be opened once, focused if already open, merged into tab groups, split by edge-drop, closed, and later pulled back out. Keep implementation differences internal; user-facing drag/drop behavior and preview must be shared.
...
- Editor leaf drop preview: use the shared thin `NotesWorkspaceDropIndicator`, same as shell modules. Do not render full-pane blue fill/border overlays during tab drag.
...
- i18n boundary: stable UI labels belong in `app/src/locales/ru.json` and `app/src/locales/en.json`; entity names/descriptions/properties are world content and should not be translated at render time. Player clients should receive world locale overrides from Yjs `worldLocales`, not by calling `/api/world/locales/*`.
