...
- Перетащить заголовок панели и вкладку на край другой панели: должна появиться только проектная drop-zone без browser ghost preview, после отпускания должен создаться split; header split-кнопок быть не должно.
