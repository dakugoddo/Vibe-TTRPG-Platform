...
- Перетащить заголовок панели и вкладку на край другой панели: должна появиться только тонкая проектная drop-zone без browser ghost preview и без большого full-pane overlay, после отпускания должен создаться split; header split-кнопок быть не должно.
