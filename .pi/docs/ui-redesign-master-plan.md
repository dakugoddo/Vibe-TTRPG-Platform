...
## 1.1 Owner decisions 2026-05-31

Ответы владельца превращают документ из открытого brainstorm в рабочий контракт для следующего UI-среза:

1. Темы - это не простая смена световой гаммы. Тема должна менять визуальный образ рабочего пространства: универсальное стекло, деревянный tabletop, RGB/game desk, arcane/tech desk, low-load вариант и будущие моддерские стили.
2. Отдельная светлая тема сейчас не нужна. Сначала делаем разные стилевые темы без обязательной пары light/dark; желающие позже смогут делать светлые/тёмные варианты сами.
3. Плотность по умолчанию - `balanced`.
4. Оформление характеристик должно балансировать tiles и compact table: важные числа читаются как карточки, длинные массивы остаются плотными и сканируемыми.
5. Отдельный "боевой режим" чарника не нужен. Нужен компактный card view для уже существующей карточки персонажа на canvas: быстрые статы, вкладки способностей/атак/ресурсов и удобные переключатели для боя.
6. Для списков предметов/атак/способностей нужны оба вида: строгая таблица с большим числом параметров и более наглядная карточная сетка с ключевыми параметрами.
7. Notes должны тянуться к Obsidian-like workspace. Нужны улучшенные окна: несколько сущностей рядом, 2-4 окна по углам, полноценная работа с заметками; в будущем - multi-window/multi-monitor режим для нескольких экранов.
8. Storage тем/переводов: пользовательский выбор и кастомные переводы можно хранить в мире, чтобы моддеры и авторы миров могли переносить настройки вместе с миром.
9. Пользовательский редактор переводов нужен, но без переусложнения. Главное - удобный моддинг, custom variables/labels для домашних систем и понятный импорт/экспорт.
10. Визуальные эффекты зависят от темы, но без вырвиглаза. Для слабых машин отдельный low-load theme/effect mode может отключать тяжёлые blur/animation/effects.
11. Layout должен быть адаптивным, но с максимальными оптимальными ширинами/высотами, чтобы окно на весь монитор не растягивало контент уродливо.
12. UI-срез считается стратегическим: дальнейшая разработка не должна уходить далеко вперёд, пока интерфейсный фундамент не станет цельным и стабильным.

Дополнительные решения владельца 2026-06-03 по workspace:

13. Незакреплённая сущность на экране открывается в одном личном экземпляре; повторное открытие фокусирует уже открытое screen-окно.
14. Закреплённые окна, фишки и карточки на canvas считаются отдельными canvas placements. Одну сущность можно закреплять сколько угодно раз без копирования entity-файла.
