...
- `Notes workspace mode`: `workspaceMode.ts` + `workspaceModeStore.ts` own the local `canvas | notes` mode; `App.tsx` switches between `InfiniteCanvas` and `NotesWorkspace`; `WindowManager` hides canvas-pinned placements when `showPinned=false`; pure tab/split data contract, including tab move/reorder, split resize ratio, tab view modes and leaf-open behavior, lives in `notesWorkspaceLayout.ts`; local tab board state lives in `notesWorkspaceStore.ts`, persists via validated `localStorage`, and migrates legacy `markdown` view to `preview`; `NotesWorkspace.tsx` is now the Obsidian-like local editor workspace with vault tree, source/preview/split Markdown modes, entity data view, children/attached entities and linked context; linked data parses headings/wiki links/backlinks in `notesWorkspaceLinks.ts`; `Editor` is the required non-toggleable shell module in `notesWorkspaceModules.ts`, not a separate central JSX branch. This path is local-only: do not add world-shared workspace layout, browser popout or native multi-window without the Electron/native gate, and do not reintroduce screen-window layout controls as the main notes workflow. Notes entity open from Vault/Search/links should use `openTabInNewLeaf()`: reuse/focus an already-open entity, but open a new entity as a sibling editor leaf. Entity editor leaves merge by center-drop and split by edge-drop.
...
- Notes shell dock areas must not own scrolling. Module/window bodies own their own `overflow-y-auto`, and the editor tab bar is the only group-level chrome; do not reintroduce a duplicate `G1/entity` header above the tab strip.
- Notes shell side-by-side module drops require `notesWorkspaceStore.shell.moduleLayouts`: left/right edge drops set the target area's layout to `row`, top/bottom edge drops set it to `column`. Drop preview comparison must include `placement/layout`, not only `area/beforeModuleId`, because the same insertion point can represent a different visual split.
- Editor leaf split is an edge-drop interaction. Do not reintroduce header buttons like `Разделить вправо/вниз`; tab/leaf center-drop merges into a tab group and edge-drop creates the split.
- Embedded Notes `Audio` should show one shell header only. Keep `AudioControlDock` as the single playback owner and use compact embedded chrome for `AudioControlDock`/`AudioDesk`; do not stack shell header + dock header + desk header.
- Notes entity opening should call `notesWorkspaceStore.openTabInNewLeaf()` from user-facing flows. Keep `openTab()` for explicit same-group tab behavior/tests; otherwise new entities will collapse back into the transitional single editor tab-strip and break the Obsidian-like leaf model.
