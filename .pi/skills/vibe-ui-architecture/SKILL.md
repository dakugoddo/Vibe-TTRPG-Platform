...
- `Compact character card`: `app/src/utils/characterCardSummary.ts` owns character summary data, `app/src/utils/characterCompactCardDefaults.ts` owns `entity.properties.compactCard` visible field defaults, `app/src/utils/entityActionRollModel.ts` + `app/src/services/entityActionRoll.ts` own roll formulas/chat dispatch for full entity sheets, and `InfiniteCanvas.tsx` owns the unified DOM overlay tabs (`Статы`, `Действия`, `Ресурсы`, `Заметки`) for both `Фишка` and `Карточка`. Canvas compact card is informational only: show formulas/data, but do not add roll/edit/add/delete controls there. Use the entity window for character interactions and editing.
...
- `Notes workspace mode`: `workspaceMode.ts` + `workspaceModeStore.ts` own the local `canvas | notes` mode; `App.tsx` switches between `InfiniteCanvas` and `NotesWorkspace`; `WindowManager` hides canvas-pinned placements when `showPinned=false`; named local snapshots live in `windowStore.ts` and save only unpinned screen windows; pure tab/split data contract, including tab move/reorder and split resize ratio, lives in `notesWorkspaceLayout.ts`; local tab board state lives in `notesWorkspaceStore.ts`, persists via validated `localStorage`, and only opens/focuses existing screen windows; read-only linked views parse headings/wiki links/backlinks in `notesWorkspaceLinks.ts`. This path is local-only and must not create a parallel entity editor, world-shared workspace format, or browser popout path before the Tauri/native gate.
