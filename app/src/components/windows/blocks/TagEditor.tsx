...
export function TagEditor({ entity }: TagEditorProps) {
    ...
    return (
        <div className="mt-4">
                ...
                {modifiers.map((mod, idx) => (
                    <div key={`${mod.path.join('.')}:${idx}`} className="flex items-center gap-2 bg-[#2a2d3d]/40 p-2 rounded-xl border border-[#2a2d3d] shadow-sm">
                        <input
                            ...
                            placeholder={t('tagEditor.pathPlaceholder')}
                        ...
                        >
                            <option value="add" className="bg-gray-900">{t('tagEditor.modifierTypes.add')}</option>
                            <option value="multiply" className="bg-gray-900">{t('tagEditor.modifierTypes.multiply')}</option>
                            <option value="min" className="bg-gray-900">{t('tagEditor.modifierTypes.min')}</option>
                            <option value="max" className="bg-gray-900">{t('tagEditor.modifierTypes.max')}</option>
                        </select>
                        <input
                            type="number"
                            value={mod.value}
            ...
            >
                <Plus size={14} /> {t('tagEditor.addModifier')}
            </button>
        </div>
    );
}
